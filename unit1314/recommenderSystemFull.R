...
mHat_svd = udvT$u %*% t(udvT$v) # notice the inner product..
...
mapF = function(mapReduceData){
  ...
  foreach(indx = 1:nrow(map), .combine = c) %do% 
    drop(mat1Obj[map[indx,1],] %*% mat2Obj[map[indx,2],])  
}
...
reduceF = function(x,n_row){
  matrix(x,nrow=n_row,byrow=FALSE)
}
(mHat_svd_approx_par = reduceF(mapF(mapReduceData),n_row))
mHat_svd_approx
